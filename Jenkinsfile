...
node('linux') {
    ...
    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AWS-Password-ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    ...
                    sh 'aws cloudformation create-stack --stack-name final-test --region us-east-1 --template-body file://docker-single-server.json --parameters ParameterKey=KeyName,ParameterValue=test ParameterKey=YourIp,ParameterValue=$(curl ifconfig.me)/32'
